...
class AgendaTest(unittest.TestCase):
    ...
    def test_get_weekly_schedule(self):
        self.assertEqual(
            len(do.get_weekly_schedule("1", datetime.now()).courses), 2
        )
...
class NextLectureTest(unittest.TestCase):
    def test_get_next_lecture(self):
        self.assertEqual(
            type(do.get_next_lecture("1", datetime.now(), "Information Ret")),
            Lecture,
        )
        self.assertEqual(
            do.get_next_lecture("1", datetime.now(), "Information Ret").course,
            "Information retrieval - t2",
        )
